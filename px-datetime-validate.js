 ...
 */
PxDatetimeBehavior.Validate = [AppLocalizeBehavior, {
  properties: {
    /**
  ...
  _validateInput: function (format, tz) {
    var inputArr = dom(this.root).querySelectorAll('.cell'),
  ...
  _validateField: function (funcOrigin) {
    var fieldNodes = new Object(),
        dateEntry = dom(this.root).querySelector('#date'),
        timeEntry = dom(this.root).querySelector('#time');

    if (dateEntry !== null) { fieldNodes.dateCells = dom(dateEntry.root).querySelectorAll('.cell') };
    if (timeEntry !== null) { fieldNodes.timeCells = dom(timeEntry.root).querySelectorAll('.cell') };
  ...
  _validField: function (submittedMoment) {
    if (!this.hideDate) {
      var dateEntry = dom(this.root).querySelector('#date');
      if(dateEntry !== null){dateEntry.toggleClass('validation-error', false, dateEntry.$.wrapper);}
    }
    if (!this.hideTime) {
      var timeEntry = dom(this.root).querySelector('#time');
  ...
  _entryInputString: function (cells) {
    ...
    for (var i = 0; i < cells.length; i++) {
      var value = dom(cells[i].root).querySelector('.datetime-entry-input').value;
      if (value === '') {
        return '';
      }
  ...
  _loopOverCellValues: function (cells) {
    ...
    if (cells && cells.length) {
      for (i = 0; i < cells.length; i++) {
        var cellInput = dom(cells[i].root).querySelectorAll('.datetime-entry-input');
